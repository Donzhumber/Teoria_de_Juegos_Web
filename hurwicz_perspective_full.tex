...
\usepackage{xcolor}
\usepackage{titling}
\usepackage[flushmargin, hang]{footmisc} % Footnotes con sangría francesa
...
% --- Estilo de Subtítulos (Simulados por el usuario) ---
...
% --- Bibliografía Elegante ---
\newcommand{\bibentry}[1]{\noindent\hangindent=2.5em\hangafter=1 \small #1 \par\vspace{0.7em}}
...
\begin{gather*}
    p^{*i} \geqq 0, \quad (x^{*i}, y^*) \text{ is individually feasible,}^1 \\
    p_i^* y^* + x^{*i} = \bar{x}^i
\end{gather*}
and
\[
...
\footnotetext{That is, $(x^{*i}, y^*)$ is in the $i$th agent's consumption set $C$, often chosen as the non-negative quadrant.}
...
\bibentry{Malinvaud, E. [1969]. ``Procédures pour la détermination d'un programme de consommation collective.'' Paper presented at the Brussels meeting of the Econometric Society, September 1969. Mimeo.}
